...
# Function to fit ridge regression model and calculate RMSE
fit_ridge <- function(data, beta) {
     ridge_model <- cv.glmnet(as.matrix(data[, c(2:4)]), data[, "poisson_y"],
                              alpha = 0, family = poisson(link = "log"), nfolds = 3)
     
     coef.ridge <- as.numeric(coef(ridge_model, s = "lambda.min")["x1", ])
     pred <- predict(ridge_model, s = "lambda.min", type = "response", newx = as.matrix(data[, c(2:4)]))
     RMSE_beta <- sqrt(mean((coef.ridge - beta)^2))
     RMSE_prediction <- sqrt(mean((data[, "poisson_y"] - pred)^2))
     
     return(list(RMSE_beta = RMSE_beta, RMSE_prediction = RMSE_prediction))
}

# Fit the ridge regression model and calculate RMSE for each replication and beta
...
for (i in seq_along(betas)) {
     ...
     ridge_results <- lapply(sim.poisson.dt.k3, fit_ridge, beta = beta)
     results[[i]] <- ridge_results
}
