...
#poisson regression model for simulation, k=3
#choose x1, x2, x4 as the target variables in the regression model
...
# Function to calculate metrics for different beta values
calculate_metrics_k3 <- function(n, betas) {
     ...
     for (i in seq_along(betas)) {
          ...
          name <- c("poisson_y", "x1", "x2", "x4")
          sim.poisson.dt.k3 <- lapply(sim.poisson.dt, function(data) data[, name])
          
          fit <- lapply(sim.poisson.dt.k3, function(data) {
          ...
          diff <- mapply(function(pred, data) pred - data[, "poisson_y"], pred, sim.poisson.dt.k3)
          ...
          results[[i]] <- list(separation = separation, RMSE_beta = RMSE_beta,
                               RMSE_prediction = RMSE_prediction, coverage.prob = count / rep)
     }
...
results_k3 <- calculate_metrics_k3(n, betas)

# Extract specific values from the results and combine them into a comma-separated string
separation_values <- paste(sapply(results_k3, function(result) result$separation), collapse = ",")
RMSE_beta_values <- paste(sapply(results_k3, function(result) result$RMSE_beta), collapse = ",")
RMSE_prediction_values <- paste(sapply(results_k3, function(result) result$RMSE_prediction), collapse = ",")
coverage_prob_values <- paste(sapply(results_k3, function(result) result$coverage.prob), collapse = ",")
