...
fit_ridge <- function(data, beta) {
     ridge_model <- cv.glmnet(as.matrix(data[, 2:6]), data[, "poisson_y"],
                              alpha = 0, family = poisson(link = "log"), nfolds = 10)
     
     coef.ridge <- as.numeric(coef(ridge_model, s = "lambda.min")["x1", ])
     pred <- predict(ridge_model, s = "lambda.min", type = "response", newx = as.matrix(data[, 2:6]))
...
for (i in seq_along(betas)) {
     ...
     ridge_results <- lapply(sim.poisson.dt.k5, fit_ridge, beta = beta)
