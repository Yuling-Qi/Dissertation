...
fit_lasso <- function(data, beta) {
     lasso_model <- cv.glmnet(as.matrix(data[, 2:8]), data[, "poisson_y"],
                              alpha = 1, family = poisson(link = "log"), nfolds = 3)
     
     coef.lasso <- as.numeric(coef(lasso_model, s = "lambda.min")["x1", ])
     pred <- predict(lasso_model, s = "lambda.min", type = "response", newx = as.matrix(data[, 2:8]))
...
for (i in seq_along(betas)) {
     ...
     lasso_results <- lapply(sim.poisson.dt, fit_lasso, beta = beta)
