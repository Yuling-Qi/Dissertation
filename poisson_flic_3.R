...
# Define a function to simulate a Poisson regression model for k=3
...
calculate_metrics_k3_firth <- function(n, betas) {
...
results_k3_firth <- calculate_metrics_k3_firth(n, betas)

# Extract specific values from the results and combine them into a comma-separated string
RMSE_prediction_values_firth <- paste(sapply(results_k3_firth, function(result) result$RMSE_prediction), 
                                      collapse = ",")
