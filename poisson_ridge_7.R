...
fit_ridge<- function(data, beta) {
     ridge_model <- cv.glmnet(as.matrix(data[, 2:8]), data[, "poisson_y"],
                              alpha = 0, family = poisson(link = "log"), nfolds = 10)
     
     coef.ridge<- as.numeric(coef(ridge_model, s = "lambda.min")["x1", ])
     pred <- predict(ridge_model, s = "lambda.min", type = "response", newx = as.matrix(data[, 2:8]))
...
# Fit the ridgeregression model and calculate RMSE for each replication and beta
results <- vector("list", length(betas))
for (i in seq_along(betas)) {
     beta <- betas[i]
     sim.poisson.dt <- lapply(1:rep, function(j) sim_poisson(n, beta))
     ridge_results <- lapply(sim.poisson.dt, fit_ridge, beta = beta)
